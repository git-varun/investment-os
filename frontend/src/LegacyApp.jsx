...
import {Activity, BrainCircuit, LayoutDashboard, LineChart, LogOut, Newspaper, Settings, Wallet} from 'lucide-react';
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <button style={navBtnStyle('terminal')} onClick={() => setActivePage('terminal')}
                        title="Terminal (Alt+1)"><LineChart size={22}/></button>
                <button style={navBtnStyle('ledger')} onClick={() => setActivePage('ledger')} title="Ledger (Alt+2)">
                    <Wallet size={22}/></button>
                <button style={navBtnStyle('analytics')} onClick={() => setActivePage('analytics')}
                        title="Risk Analytics (Alt+3)"><LayoutDashboard size={22}/></button>
                <button style={navBtnStyle('ai')} onClick={() => setActivePage('ai')} title="Global AI (Alt+4)">
                    <BrainCircuit size={22}/></button>
                <button style={navBtnStyle('news')} onClick={() => setActivePage('news')} title="News Squawk (Alt+5)">
                    <Newspaper size={22}/></button>
                <div style={{flex: 1}}/>
                <button style={{...navBtnStyle('profile'), marginBottom: '10px'}}
                        onClick={() => setActivePage('profile')} title="Control Center (Alt+6)"><Settings size={22}/>
                </button>
                <button
                    style={{
                        ...navBtnStyle('logout'),
            ...
            <div style={{flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden'}}>
                ...
                <div style={{flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
                    {activePage === 'terminal' &&
                        <Terminal state={state} selectedSymbol={selectedSymbol} setSelectedSymbol={setSelectedSymbol}
                                  showLeft={showLeft} showRight={showRight}/>}
                    {activePage === 'ledger' &&
                        <Ledger assets={state.assets} totalVal={state.total_value_inr} fx={state.fx_rate}
