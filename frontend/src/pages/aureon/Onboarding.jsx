...
function StepBar({step}) {
    return (
        <div style={{display: 'grid', gridTemplateColumns: `repeat(${STEPS.length}, 1fr)`, gap: 8}}>
            {STEPS.map((s, i) => (
                <div key={s}>
                    <div style={{height: 3, borderRadius: 2, background: i <= step ? 'var(--aurum-500)' : 'rgba(255,255,255,0.06)', transition: 'background 220ms'}}/>
                    <div style={{display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, marginTop: 6}}>
                        <span style={{
            ...
            <div style={{marginTop: 28, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14}}>
                {[
                    ...
                    <div key={b.t} className="layer-1" style={{padding: '16px 18px'}}>
                        <div style={{fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 600, color: 'var(--ink-00)'}}>{b.t}</div>
...
function LinkAccounts({linked, setLinked}) {
    ...
    return (
        <div>
            <div style={{display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 6}}>
                ...
                <div style={{fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink-30)'}}>{count} of {PROVIDERS.length} connected</div>
            </div>
                    ...
                    return (
                        <div key={p.id} className="layer-1" style={{padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14}}>
                            <div style={{
                                ...
                                fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 18, color: '#0B0D10',
                                background: p.color, flexShrink: 0,
                            }}>{p.logo}</div>
                            <div style={{flex: 1, minWidth: 0}}>
                                <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
                                    <span style={{fontFamily: 'var(--font-heading)', fontSize: 14.5, fontWeight: 600, color: 'var(--ink-00)'}}>{p.name}</span>
...
function SetGoals({goals, setGoals}) {
    ...
    return (
        <div style={{maxWidth: 760}}>
            ...
            <div className="layer-1" style={{padding: '20px 24px', marginBottom: 14}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>Retirement</div>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 14}}>
                    ...
                    <div>
                        ...
                        <div style={{fontFamily: 'var(--font-mono)', fontSize: 32, fontWeight: 500, color: 'var(--ink-00)'}}>{fmtINR(goals.corpus)}</div>
                        <input type="range" min="10000000" max="200000000" step="2500000" value={goals.corpus} onChange={e => set('corpus', +e.target.value)} style={{width: '100%', marginTop: 8, accentColor: '#C9A86A'}}/>
                    </div>
                </div>
            </div>

            <div className="layer-1" style={{padding: '20px 24px', marginBottom: 14}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>Monthly investing</div>
                <div style={{display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 14}}>
                    <span style={{fontFamily: 'var(--font-mono)', fontSize: 32, fontWeight: 500, color: 'var(--ink-00)'}}>{fmtINR(goals.monthly)}</span>
            ...
            <div className="layer-1" style={{padding: '20px 24px'}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>Risk profile</div>
...
function Review({linked, goals}) {
    const linkedNames = PROVIDERS.filter(p => linked[p.id]).map(p => p.name);
    return (
        <div style={{maxWidth: 760}}>
            ...
            <div className="layer-1" style={{padding: '18px 22px', marginBottom: 12}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>Linked accounts ({linkedNames.length})</div>
                <div style={{marginTop: 10, fontSize: 13, color: 'var(--ink-10)'}}>{linkedNames.length ? linkedNames.join(' · ') : 'None yet — add from Settings.'}</div>
            </div>
            <div className="layer-1" style={{padding: '18px 22px', marginBottom: 12}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>Goals</div>
                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 18, marginTop: 12}}>
                    ...
                    <div><div style={{fontSize: 11, color: 'var(--ink-40)'}}>Corpus</div><div style={{fontFamily: 'var(--font-mono)', fontSize: 18, color: 'var(--ink-00)', marginTop: 2}}>{fmtINR(goals.corpus)}</div></div>
                    <div><div style={{fontSize: 11, color: 'var(--ink-40)'}}>Monthly</div><div style={{fontFamily: 'var(--font-mono)', fontSize: 18, color: 'var(--ink-00)', marginTop: 2}}>{fmtINR(goals.monthly)}</div></div>
            ...
            <div className="layer-1" style={{padding: '18px 22px'}}>
                <div style={{fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-30)', fontWeight: 600}}>What happens next</div>
...
export default function Onboarding({onDone}) {
    ...
    return (
        <div style={{minHeight: '100vh', background: 'var(--canvas)', display: 'flex', flexDirection: 'column', backgroundImage: 'radial-gradient(900px 500px at 80% -200px, rgba(201,168,106,0.08), transparent 60%)'}}>
            {/* Header */}
            <div style={{padding: '24px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)'}}>
                ...
                <div style={{fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-40)'}}>Step {step + 1} of {STEPS.length}</div>
                <button onClick={finish} className="du3-cta ghost">Skip for now</button>
            </div>
            ...
            <div style={{flex: 1, padding: '36px 40px', maxWidth: 980, margin: '0 auto', width: '100%'}}>
                {step === 0 && <Welcome/>}
                {step === 1 && <LinkAccounts linked={linked} setLinked={setLinked}/>}
                {step === 2 && <SetGoals goals={goals} setGoals={setGoals}/>}
                {step === 3 && <Review linked={linked} goals={goals}/>}
            </div>
            ...
            <div style={{padding: '20px 40px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.015)'}}>
                <button onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0} className="du3-cta ghost" style={{opacity: step === 0 ? 0.4 : 1}}>← Back</button>
                <div style={{fontSize: 11, color: 'var(--ink-40)'}}>You can change any of this later in Settings.</div>
                {step < STEPS.length - 1 ? (
                    <button onClick={() => setStep(s => s + 1)} className="du3-cta">Continue →</button>
                ) : (
                    <button onClick={finish} className="du3-cta">Enter Aureon →</button>
                )}
            </div>
        </div>
    );
