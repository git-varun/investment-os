...
export default function AssetDetail({ticker, go}) {
    ...
    const events = (() => {
        ...
        return [{i: Math.floor(series.length * 0.45), label: 'Trim · 5/4'},
            ...
            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 22}}>
                ...
                <section className="layer-1" style={{padding: '14px 18px'}}>
                        ...
                        <div style={{display: 'grid', gap: 8}}>
                            {sigs.map(s => (
                                <div key={s.id} style={{
                                    ...
                                    {s.linkedRec &&
                                        <button onClick={() => go('recommendations')} className="du3-cta ghost"
                    ...
                    <div style={{
                        ...
                        Signals are inputs. Decisions live in <button onClick={() => go('recommendations')}
            ...
            <section className="layer-1" style={{padding: '14px 18px', marginTop: 14}}>
                ...
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 24, marginTop: 12}}>
                    {[
                        ...
                        ['Avg cost', '$' + h.cost.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                        })],
                        ['Value', '$' + Math.round(v).toLocaleString()],
                        ['Unreal P/L', (pl >= 0 ? '+' : '−') + '$' + Math.round(Math.abs(pl)).toLocaleString() + ' · ' + (plPct >= 0 ? '+' : '−') + (Math.abs(plPct) * 100).toFixed(1) + '%'],
