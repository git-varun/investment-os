/* ============================================================
   Aureon — Signals refactor (dense rows → expandable cards)
   ============================================================ */

/* Direction inference (bull/bear/neutral) based on signal kind + text */
...
/* Confidence synthesized from severity */
const sigConfidence = (s) => s.severity === 'high' ? 78 + (s.id.charCodeAt(3) % 10) : s.severity === 'med' ? 60 + (s.id.charCodeAt(3) % 12) : 42 + (s.id.charCodeAt(3) % 14);

/* Detailed reasoning (mock — would be from the model) */
const detailedReasoning = (s) => {
  const d = inferDirection(s);
  const base = {
...
const ConfidenceBar = ({ v }) => (
  <div style={{display:'flex',alignItems:'center',gap:8,minWidth:120}}>
    <div style={{flex:1,height:4,borderRadius:999,background:'rgba(255,255,255,0.06)',overflow:'hidden'}}>
      <div style={{width:`${v}%`,height:'100%',background: v>=70 ? 'var(--aurum-500)' : v>=50 ? 'var(--dusk-500)' : 'var(--ink-30)',borderRadius:'inherit'}}/>
    </div>
    <span style={{fontFamily:'var(--font-mono)',fontSize:11,color:'var(--ink-20)',width:28,textAlign:'right'}}>{v}</span>
  </div>
);

/* ---------- AskSignalPanel — same Q&A panel adapted for signals ---------- */
const AskSignalPanel = ({ s }) => {
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const ctx = `Signal: ${s.kind} on ${s.asset}. Severity: ${s.severity}. Detection: "${s.text}".`;
      const text = await window.claude.complete({
        messages:[{ role:'user', content:`You are Aureon, a concise AI investment advisor. Context: ${ctx}\n\nUser question: ${q}\n\nAnswer in 1–3 sentences. Be specific to this signal.` }]
      });
    ...
    <div style={{borderTop:'1px dashed rgba(255,255,255,0.08)',marginTop:8}}>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--ink-40)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
             style={{transform:open?'rotate(180deg)':'rotate(0deg)',transition:'transform 160ms var(--ease-std)'}}>
          <polyline points="6 9 12 15 18 9"/>
        </svg>
      </button>
      {open && (
        <div style={{paddingBottom:14,animation:'cardEnter 200ms ease'}}>
  ...
  return (
    <article className="layer-1" style={{
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:18,alignItems:'center'}}>
        <p style={{margin:0,fontSize:13.5,color:'var(--ink-10)',lineHeight:1.55,letterSpacing:'-0.005em',textWrap:'pretty'}}>{s.text}</p>
        <div>
      ...
      <footer style={{display:'flex',alignItems:'center',gap:8,marginTop:14,paddingTop:12,borderTop:'1px solid rgba(255,255,255,0.05)'}}>
        ...
        <div style={{flex:1}}/>
        {s.linkedRec ? (
          <button onClick={() => go('recommendations')} className="du3-cta" style={{padding:'4px 12px',fontSize:11.5}}>View recommendation →</button>
        ) : (
          <span style={{fontSize:11,color:'var(--ink-40)'}}>No action · informational</span>
        )}
      </footer>

      {/* Expanded: technical reasoning */}
      {open && (
        <div style={{marginTop:12,padding:'12px 14px',borderRadius:8,background:'rgba(0,0,0,0.20)',border:'1px solid rgba(255,255,255,0.05)',animation:'cardEnter 220ms var(--ease-decel)'}}>
...
const Signals = ({ go }) => {
  ...
  // Group by asset for visual separation
  ...
  return (
    <>
      <div style={{padding:'12px 16px',marginBottom:18,borderRadius:10,background:'rgba(212,162,87,0.06)',border:'1px solid rgba(212,162,87,0.20)',fontSize:12.5,color:'var(--ink-10)',display:'flex',alignItems:'center',gap:10}}>
      ...
      <div style={{display:'flex',gap:32,alignItems:'flex-end',paddingBottom:18,marginBottom:20,borderBottom:'1px solid rgba(255,255,255,0.05)',flexWrap:'wrap'}}>
        ...
        <div>
          ...
          <div style={{fontFamily:'var(--font-mono)',fontSize:22,fontWeight:500,color:'var(--aurum-100)',marginTop:6}}>{SIGNALS.filter(s=>s.linkedRec).length}</div>
        </div>
        <div>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          <select value={kind} onChange={e => setKind(e.target.value)} style={selectStyleV3}>
            {kinds.map(k => <option key={k} value={k}>{k==='all'?'All kinds':k}</option>)}
          </select>
          <select value={sev} onChange={e => setSev(e.target.value)} style={selectStyleV3}>
            {sevs.map(k => <option key={k} value={k}>{k==='all'?'All severities':k}</option>)}
          </select>
          <select value={dir} onChange={e => setDir(e.target.value)} style={selectStyleV3}>
      ...
      {/* Grouped cards */}
      {grouped.length === 0 ? (
        <Empty>No signals match the filters.</Empty>
        ...
        <section key={asset} style={{marginBottom:24}}>
          ...
          <div style={{display:'grid',gap:10}}>
            {items.map(s => <SignalCard key={s.id} s={s} go={go}/>)}
          </div>
        </section>
      ))}
...
const selectStyleV3 = {
  ...
  color:'var(--ink-10)', fontFamily:'var(--font-ui)', colorScheme:'dark', cursor:'pointer', outline:'none',
};

Object.assign(window, { Signals, SignalCard, AskSignalPanel });
