...
class QuantEngine:
    ...
    def _calculate_risk_metrics(self, returns: pd.Series, benchmark_returns: pd.Series = None) -> dict:
        ...
        if returns.empty or returns.std() == 0: return metrics
        ...
        return {k: round(v, 3) if not np.isnan(v) and not np.isinf(v) else 0.0 for k, v in metrics.items()}
    ...
    def analyze_asset(self, symbol: str, current_price: float, total_portfolio_value: float = 100000, prices_history: list = None) -> dict:
        ...
        if symbol == "GOLD" or symbol.startswith("FD-") or "_MF" in symbol: return metrics

        try:
            # Use passed price history (caller responsible for fetching from repository)
...
from app.modules.analytics.services import QuantEngine as ModuleQuantEngine

QuantEngine = ModuleQuantEngine
