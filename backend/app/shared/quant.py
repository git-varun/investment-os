...
class QuantEngine:
    ...
    def compute_for_asset_type(self, asset_type: str, prices=None, metadata: dict = None) -> dict:
        """Route to the appropriate quant computation based on asset type."""
        if asset_type in ("equity", "crypto", "commodity"):
            return self.compute_all(prices) if prices else {}
        elif asset_type == "bond":
            return self.compute_bond_metrics(metadata or {})
        elif asset_type in ("epf", "ppf"):
            return self.compute_fixed_return_metrics(metadata or {})
        elif asset_type == "real_estate":
            return self.compute_real_estate_metrics(metadata or {})
        return {}

    def compute_bond_metrics(self, metadata: dict) -> dict:
        """YTM (Newton-Raphson), modified duration, accrued interest."""
        import datetime
        try:
            ...
            maturity_date_str = metadata.get("maturity_date")
            if not maturity_date_str:
                return {"coupon_rate": metadata.get("coupon_rate"), "accrued_interest": 0.0}

            maturity = datetime.date.fromisoformat(maturity_date_str)
            today = datetime.date.today()
            years_to_maturity = (maturity - today).days / 365.25
            if years_to_maturity <= 0:
                return {"ytm": coupon_rate, "modified_duration": 0.0, "accrued_interest": 0.0}

            n_periods = int(years_to_maturity * freq)
            # Days since last coupon payment
            days_in_period = 365.25 / freq
            days_since_coupon = (today - maturity).days % int(days_in_period)
            accrued_interest = round(coupon * (days_since_coupon / days_in_period), 4)

            # Newton-Raphson YTM (use face_value as current price if no market price)
            current_price = float(metadata.get("current_price", face_value))
            ytm = coupon_rate  # initial guess
            for _ in range(100):
                pv = sum(coupon / (1 + ytm / freq) ** t for t in range(1, n_periods + 1))
                pv += face_value / (1 + ytm / freq) ** n_periods
                dpv = sum(-t * coupon / freq / (1 + ytm / freq) ** (t + 1) for t in range(1, n_periods + 1))
                dpv -= n_periods * face_value / freq / (1 + ytm / freq) ** (n_periods + 1)
                diff = pv - current_price
                if abs(diff) < 1e-6:
                    break
                ytm -= diff / dpv if dpv != 0 else 0

            # Modified duration
            macaulay = sum(
                (t / freq) * (coupon / (1 + ytm / freq) ** t)
                for t in range(1, n_periods + 1)
            )
            macaulay += (n_periods / freq) * (face_value / (1 + ytm / freq) ** n_periods)
            if current_price > 0:
                macaulay /= current_price
            ...
            logger.warning("compute_bond_metrics failed: %s", exc)
            return {}

    def compute_fixed_return_metrics(self, metadata: dict) -> dict:
        """Effective rate and projected 5-year corpus for EPF/PPF."""
        try:
            rate = float(metadata.get("interest_rate", 0.0)) / 100.0
            ...
            monthly_total = employee + employer + vpf
            # Future value of annuity: FV = P * ((1+r)^n - 1) / r * (1+r)
            r_monthly = rate / 12
            n = 60  # 5 years
            if r_monthly > 0:
                corpus_5y = monthly_total * ((1 + r_monthly) ** n - 1) / r_monthly * (1 + r_monthly)
            else:
                corpus_5y = monthly_total * n
            return {
                "effective_rate": round(rate * 100, 2),
                "monthly_contribution": round(monthly_total, 2),
                "corpus_projection_5y": round(corpus_5y, 2),
            }
        except Exception as exc:
            logger.warning("compute_fixed_return_metrics failed: %s", exc)
            return {}

    def compute_real_estate_metrics(self, metadata: dict) -> dict:
        """Rental yield, cap rate, price per sqft."""
        try:
            rental_monthly = float(metadata.get("rental_monthly", 0.0))
            annual_rental = rental_monthly * 12
            area = float(metadata.get("area_sqft") or 0.0)
            mortgage = float(metadata.get("mortgage_outstanding", 0.0))
            current_value = float(metadata.get("current_value", 0.0))

            rental_yield = (annual_rental / current_value * 100) if current_value > 0 else 0.0
            # Simplified cap rate (no operating expenses tracked)
            cap_rate = rental_yield
            price_per_sqft = (current_value / area) if area > 0 else 0.0
            ...
            logger.warning("compute_real_estate_metrics failed: %s", exc)
            return {}
