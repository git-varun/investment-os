...
def parse_symbols_field(symbols_str: str) -> List[str]:
    """Parse a comma-separated symbols string into a cleaned list of uppercase symbols."""
    if not symbols_str:
        return []
    ...
    def fetch_and_store(self, symbol: str, db: Session, is_crypto: bool = False) -> int:
        ...
        if new_count:
            try:
                ...
                # Dual-write: link newly inserted news rows to assets via junction table
                _link_news_assets(db, symbol)
            except Exception as exc:
                db.rollback()
                logger.error(
...
def _link_news_assets(db: Session, symbol: str) -> None:
    """Insert news_assets rows for news articles whose symbols field contains *symbol*.

    Idempotent — uses ON CONFLICT DO NOTHING via SQLAlchemy merge pattern.
    Called after fetch_and_store commits new articles.
    """
    ...
    recent_news = (
        db.query(News)
        .filter(News.symbols.ilike(f"%{symbol}%"))
    ...
    if linked:
        try:
            db.commit()
            logger.debug("_link_news_assets: symbol=%s linked %d news_assets rows", symbol, linked)
        except Exception as exc:
            db.rollback()
            logger.warning("_link_news_assets: commit failed for symbol=%s: %s", symbol, exc)
