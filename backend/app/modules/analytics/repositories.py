        ...
        with get_db().transaction() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                                ...
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (asset_id, ts) DO
                                UPDATE SET
        ...
        with get_db().transaction() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                                ...
                                (asset_id, momentum_rsi, trend_strength, price_risk_pct, bb_upper, bb_lower, vwap,
                                 z_score, macro_tsl, target_1_2, tv_signal, bmsb_status, ts)
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                            """,
                            (asset_id, momentum_rsi, trend_strength, price_risk_pct, bb_upper, bb_lower, vwap, z_score,
                             macro_tsl, target_1_2, tv_signal, bmsb_status, ts))
                ...
                for r in records:
                    cur.execute("""
                                ...
                                (r['asset_id'], r.get('momentum_rsi'), r.get('trend_strength'), r.get('price_risk_pct'),
                                 r.get('bb_upper'), r.get('bb_lower'), r.get('vwap'), r.get('z_score'),
                                 r.get('macro_tsl'),
                                 r.get('target_1_2'), r.get('tv_signal'), r.get('bmsb_status'),
