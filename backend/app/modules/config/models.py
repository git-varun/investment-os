...
class ProviderConfig(Base):
    ...
    config = Column(Text, default="{}")  # JSON blob for non-credential settings
...
class AllocationTarget(Base):
    """Per-asset-class target allocation (Aureon).

    `asset_class` is a free-form key matching Aureon's CLASS_LABEL keys
    (stocks, crypto, funds, bonds, real_estate, retirement, insurance).
    """

    __tablename__ = "allocation_targets"

    id = Column(Integer, primary_key=True, index=True)
    asset_class = Column(String(40), unique=True, nullable=False, index=True)
    target_pct = Column(Integer, nullable=False)  # stored as basis-points (0-10000) for precision
    band_low_pct = Column(Integer, nullable=True)
    band_high_pct = Column(Integer, nullable=True)
