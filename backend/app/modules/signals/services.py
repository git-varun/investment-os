...
class SignalService:
    ...
    def generate_signals_batch(self, symbols: Optional[List[str]] = None, user_id: Optional[int] = None) -> List[Signal]:
        ...
        """
        # Read signal-eligible types from DB config (falls back to tradeable defaults)
        eligible_types = self._get_eligible_asset_types()
        ...
        logger.info("generate_signals_batch: generating signals for %d symbols (eligible=%s)",
                    len(symbols), eligible_types)
    ...
    def _get_eligible_asset_types(self) -> set:
        """Read signal-eligible asset types from DB config; fall back to tradeable defaults."""
        try:
            import json
            from app.modules.config.models import ProviderConfig
            row = self.session.query(ProviderConfig).filter_by(provider_name="signal_eligibility").first()
            ...
            logger.warning("_get_eligible_asset_types: DB read failed (%s), using defaults", exc)
        return {"equity", "crypto", "commodity"}
