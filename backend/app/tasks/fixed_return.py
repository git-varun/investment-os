    ...
    try:
        ...
        for asset in assets:
            try:
                ...
                if not provider:
                    logger.warning("bond_mtm: no valuation provider for bonds, skipping")
                    break
                ...
                pos = session.query(Position).filter(Position.asset_id == asset.id).first()
                if pos:
                    pos.current_value = round(value * pos.quantity, 2)
                    pos.last_valued_at = datetime.now(timezone.utc)

                asset.current_price = round(value, 4)
                session.add(AssetValuation(
                    asset_id=asset.id,
                    valuation_amount=round(value * (pos.quantity if pos else 1), 2),
